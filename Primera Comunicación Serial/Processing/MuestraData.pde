...
IntList info; //Arreglo para almacenar los datos de la serial
...
void draw(){
    ...
    paint(ScalaAmpSenal*info.remove(0)); //Se pintan
    }
}
...
void paint(int punto){
    ...
    line(x1,y1*scaleAmp+posCeroSenal,x2,y2*scaleAmp+posCeroSenal);
    x1 = x1 + 4*scaleTime;
    x2 = x2 + 4*scaleTime;
    y1 = y2;
...
void grid(){
  ...
  while(i<50){
     ...
     fill(0);
     text((-i*spaceH+height/2-90+posCeroH)/100, 0, i*spaceH);
     i=i+1;
  }
