...
void main(void)
{
  /* Write your local variable definition here */
	...
	volatile VALOR iADC, iADC2,iADCt1,iADCt2, tr1, tr2; 
	volatile int digital1;
	volatile int digital2;
	  ...
	  if(estado == ENVIAR){
			...
			//Digital 1  
					if (Bit0_GetVal())
						{digital1 = 0x3F ;
						  eADC[1] = eADC[1] &  0x3F;
						 PWM5_SetRatio16(0x6666); 
						 PWM6_SetRatio16(0x0000);
						}
					else {digital1 = 0x1F ;
					PWM5_SetRatio16(0x0000);
					eADC[1] = eADC[1] &  0x1F;}
					//eADC[1] = eADC[1] & 7F ;
		   ...
		   //Digital 2  
					if (Bit1_GetVal())
						...
						eADC[3] = eADC[3] & 0x3F;
						PWM5_SetRatio16(0x0000) ; 
						PWM6_SetRatio16(0x6666) ;}
					else {digital2 = 0x1F ;
					eADC[3] = eADC[3] & 0x1F;
					PWM6_SetRatio16(0x0000);}	
					//eADC[3] = eADC[3] & 7F ;
					//eADC[3] = eADC[3] & digital2;
			...
			//PWM CHANNEL 1  (Left motor )
					
					if (iADCt1.u16 > 0x7530){
						//do{error = ;}while((error != ERR_OK));
						PWM1_SetRatio16(0xB333) ;
						PWM2_SetRatio16(0x0000) ;
					}
					else{
						if (iADCt1.u16 < 0x6D30){
							PWM1_SetRatio16(0x0000) ;
							PWM2_SetRatio16(0xB333) ;}
						else {
								PWM1_SetRatio16(0x0000);
								PWM2_SetRatio16(0x0000);
						}
					 }
					//PWM CHANNEL 2  (Right Motor)
					if (iADCt2.u16 > 0x8BB8){
						//do{error = ;}while((error != ERR_OK));
						PWM3_SetRatio16(0xB333);
						PWM4_SetRatio16(0x0000);
										}
					else{
						if (iADCt2.u16 < 0x6D30){
							PWM3_SetRatio16(0x0000);
							PWM4_SetRatio16(0xB333) ;}
					else {
							PWM3_SetRatio16(0x0000) ;
							PWM4_SetRatio16(0x0000) ;
					}					}
				
					// Send channels through serial port
			 do{
